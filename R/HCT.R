...
#' gg<-HCTSurvDesign(k=c(0.33333,0.66667,1),alpha=0.05,beta=0.1,delta=1/1.75,d1=65,option="OBF")
#' @import stats
#' @references
...
HCTSurvDesign<-function(k, alpha, beta, delta, d1, option,param){
  while(d1>0) {
  ...
  ans=list(Design=data.frame(Looks=1:length(ts),Events=round(d2*k)),Efficacy=data.frame("Information time"=round(ts,4),"Cumulative alpha spent"=find$alpha,"Efficacy boundary in z-score scale"=ub,
           ...
           Futility=data.frame("Information time"=round(ts,4),
                               "Cumulative beta spent"=find$beta,"Futility boundary in z-score scale"=lb,"Futility boundary in p-value scale"=k2,"Boundary crossing probability"=p2, check.names = FALSE),etam=etam,d2max=d2,delta_used=delta)
  return(ans)}

HCTSurvDesign<-set.defaults(HCTSurvDesign,option="OBF",param=4)
#################################################################
