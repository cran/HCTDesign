...
HCTSurvDesign<-function(k, alpha, beta, delta, d1, option,param,trial,delta0){
  ...
  for (i in 2:length(lb)){
    ...
    l1[i]<-mvtnorm::pmvnorm(lower=c(lb[1:(i-1)],ub[i]),upper=c(ub[1:(i-1)],Inf),sigma=find$sig[2:(i+1),2:(i+1)],mean = etam*sqrt(ts[1:i]))[1]
    l2[i]<-mvtnorm::pmvnorm(lower=c(lb[1:(i-1)],-Inf),upper=c(ub[1:(i-1)],lb[i]),sigma=find$sig[2:(i+1),2:(i+1)])[1]
  }
  k1<-NULL
  k2<-NULL
  ...
  ans=list(Design=data.frame(Looks=1:length(ts),Events=round(d2*k)),Efficacy=data.frame("Information time"=round(ts,4),"Cumulative alpha spent"=find$alpha,"Efficacy boundary in z-score scale"=ub,
                                                                                        "Efficacy boundary in p-value scale"=k1,"Boundary crossing probability under H0"=p1,"Boundary crossing probability under H1"=l1, check.names = FALSE),
           Futility=data.frame("Information time"=round(ts,4),
                               "Cumulative beta spent"=find$beta,"Futility boundary in z-score scale"=lb,"Futility boundary in p-value scale"=k2,"Boundary crossing probability under H0"=l2,"Boundary crossing probability under H1"=p2, check.names = FALSE),etam=etam,d2max=d2,delta_used=delta,trial=trial)
  return(ans)}
